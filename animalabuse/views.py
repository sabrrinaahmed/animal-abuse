...
def profile_upload(request):
    ...
    for column in csv.reader(io_string, delimiter=',', quotechar='"'):
            _, created = animalabuse.objects.update_or_create(
                ...
                image = column[9],
                dataSource = column[10],
                sourceDescription = column[11],
                sourceLink = column[12]
...
def search(request):
    ...
    # Make tuples for data source
    SOURCE_TYPE = (('Government Sourced Entries', 'Government Sourced Entries'),
               ('Publicly Entered Entries', 'Publicly Entered Entries'))
    SOURCE_TYPE_DIC = dict(SOURCE_TYPE)

    source_list = []
    field_object = animalabuse._meta.get_field('dataSource')
    for obj in user_list:
        field_value = field_object.value_from_object(obj)
        if field_value not in source_list:
            source_list.append(field_value)

    # State dict
    sources = dict((src, srcs) for src, srcs in SOURCE_TYPE_DIC.items() if src in source_list) 
    ...
    source_query = request.GET.get('dataSource')
    ...
    if is_valid_queryparam(source_query) :
        user_list = user_list.filter(dataSource = source_query)
...
def submitnew(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            dataSource = "Publicly Entered Entries"
            sourceDescription = "Publicly Entered Entries"

            p = animalabuse(name=name.upper(), Age=Age, county=county.upper(), state=state, 
                Offense=Offense.upper(),convictiondate = convictiondate, dataSource = dataSource,
